function [p, eps2] = b04jcdf(Hd,Scf,Hs,Tz,tail)
%B04JCDF  Brodtkorb (2004) joint (Scf,Hd) CDF from Japan Sea.
%
% CALL: F = b04jcdf(Hd,Scf,Hs,Tz,tail)
%
%    F  = CDF
...
%  sc = 0.25;
%  hc = 4;
%  p = b04jcdf(hc,sc,Hs,Tz) % Prob(Hd<=hc,Scf<=sc|Hs,Tz) = 0.66
%  
%  % Conditional probability of steep and high waves given seastates
%  % i.e., Prob(Hd>hc,Scf>sc|Hs,Tz)  
%  upperTail = 1;
%  Hs = linspace(2.5,11.5,17);
%  Tz = linspace(4.5,11,16);
%  [T,H] = meshgrid(Tz,Hs); 
%  p = b04jcdf(hc,sc,H,T,upperTail);
%  v = 10.^(-6:-1);  
...
if any(k0)
  ...
  if (tail == lowerTail)
    %k       = find(h>h0);
    hlim(h>h0) = h0;
    p(k0) = gaussq(@b04jpdf,0,hlim,eps2/2,[],s,Hs,Tz,5,normalizedInput)...
	+ gaussq(@b04jpdf,hlim,h,eps2/2,[],s,Hs,Tz,5,normalizedInput); 
  else
    %k       = find(h<h0);
    hlim(h<h0) = h0;
    p(k0) = gaussq(@b04jpdf,h,hlim,eps2/2,[],s,Hs,Tz,7,normalizedInput)...
	+ gaussq(@b04jpdf,hlim,hMax,eps2/2,[],s,Hs,Tz,7,normalizedInput); 
  end
end
return
