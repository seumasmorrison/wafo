function f = ohhsscdf(Hd,Scf,Hm0,def,tail)
...
if (tail == lowerTail)
  k       = (h>2.5);
  hlim(k) = 2.5;
  f = gaussq(@ohhsspdf,0,hlim,eps2/2,[],s,Hm0,def,normalizedInput,5)...
      + gaussq(@ohhsspdf,hlim,h,eps2/2,[],s,Hm0,def,normalizedInput,5); 
else % upper tail
  k       = (h<2.5);
  hlim(k) = 2.5;
  f = gaussq(@ohhsspdf,h,hlim,eps2/2,[],s,Hm0,def,normalizedInput,7)...
      + gaussq(@ohhsspdf,hlim,hMax,eps2/2,[],s,Hm0,def,normalizedInput,7); 
end
return
