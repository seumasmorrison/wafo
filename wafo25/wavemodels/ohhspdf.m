...
if multipleSeaStates
  ...
  for iz=1:numSeaStates
    k = find(jx==iz);
    Hm0i(:) = Hm0u(iz);
    A1(k) = exp(smooth(h2,interp2(E2,H2,A11.',Hm0i,h2,method),...
		       1,h(k),1));
    B1(k) = (smooth(h2,interp2(E2,H2,exp(B11).',Hm0i,h2,method),...
		       1,h(k),1));
  end
else
  Hm0i = Hm0(ones(size(h2)));
  A1 = exp(smooth(h2,interp2(E2,H2,A11.', ...
			     Hm0i,h2,method),1,h,1));
  B1 = (smooth(h2,interp2(E2,H2,exp(B11).', ...
			  Hm0i,h2,method),1,h,1));
end
