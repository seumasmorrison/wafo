function f = jhsnlcdf(Hd,Scf,Hm0,Tp,gam,tail)
...
if 0,
  Tm02 = Tp;
  Tp = Tm02.*(polyval(c1,gam)./polyval(c2,gam));
else
...
eps2 = 1e-6;
normalizedInput = 1;
utprb = gaussq('jhsnlpdf',hMax,2*hMax,eps2/2,[],mean(v(:)),mean(Hm0(:)),mean(Tp(:)),mean(gam(:)),normalizedInput,7);
if eps2<utprb
  warning('WAFO:JHSNLCDF','Check the accuracy of integration!')
end
...
if tail==lowerTail,
  %k       = find(h>2.5);
  hlim(h>2.5) = 2.5;
  f(k0) = gaussq(@jhsnlpdf,0,hlim,eps2/2,[],v,Hm0,Tp,gam,normalizedInput,5)...
      + gaussq(@jhsnlpdf,hlim,h,eps2/2,[],v,Hm0,Tp,gam,normalizedInput,5); 
else % upper tail
  %k       = find(h<2.5);
  hlim(h<2.5) = 2.5;
  f(k0) = gaussq(@jhsnlpdf,h,hlim,eps2/2,[],v,Hm0,Tp,gam,normalizedInput,7)...
      + gaussq(@jhsnlpdf,hlim,hMax,eps2/2,[],v,Hm0,Tp,gam,normalizedInput,7); 
end
return
