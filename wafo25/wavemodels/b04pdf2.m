function p = b04pdf2(Hd,Scf,Hs,Tz,normalizedInput)
%B04PDF2 Brodtkorb (2004) joint (Scf,Hd) PDF of laboratory data.
%   
%    CALL:  f = b04pdf2(Hd,Scf,Hm0,Tm02)
%   
%       f  = density 
...
%   
%    B04PDF2 returns the joint PDF of (Scf, Hd) given Hm0 and Tm02,
...
[X,Y] = meshgrid(Scf,Hd);
p.f   = b04pdf(Y,X,Hs,Tz,0,normalizedInput);
p.x{1} = Scf(:);
p.x{2} = Hd(:);
...
p.title='Brodtkorb (2004) density of crest front steepness and wave height.';
