function [CI,Lp,nphat]=ciproflog(phat1,varargin)
%CIPROFLOG Confidence Interval using Profile Log- likelihood or Product Spacing- function.
% 
% CALL:  [CI,Lp,nphats] = ciproflog(phats,options);
%
...
%             
%  CIPROFLOG is a utility function for making inferences either on a particular
...
%   phat = fitweib(R)         
%   opt = ciproflog('defaults');
%   opt.i = 2;opt.plotflag=1;
%   [CI,Lp] = ciproflog(phat,opt)
%   [x,xlo,xup] = invweib(1/990,phat,'lowertail',false);
%   [CI0,Lp0] = ciproflog(phat,'i',2,'x',x,'link',@lnkweib,'plot',1);
%
...
%   [CI2,Lp2] = ciproflog(phat2,'i',2,'x',x,'link',@lnkgenpar,'plot',1);
%   [CI3,Lp3] = ciproflog(phat2,'i',2,'logR',-log(9900),'link',@lnkgenpar,'plot',1);
%
...
if options.check
  ...
  if any(ind)
      if numel(ind)>1
          warning('Two peaks in profile!!! Something is wrong??')
      end
    p_opt = ecross((pvec(iz)+pvec(iz+1))/2,dLL,ind(1),0);
  else
    p_opt = pvec(ix);
  end
  if isempty(link)
...
switch length(ind)
  ...
  otherwise
	...
	CI = ecross(pvec,LL,ind([1,end]),cross_level);
end
if nargout>1 || (options.plotflag)
%% Prettify result
ix = find(LL==lowLevel);
LL(ix) = [];
pvec(ix) = [];
