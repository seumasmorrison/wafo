...
%SUBSREF Define field name indexing for objects or methods to apply
%
% 
% Examples
%  x = linspace(0,5); 
%  d = fdata(pdfray(x,1),x);
...
switch index(1).type
  case '()'
    ...
    elseif strcmpi(index(2).type,'.')
      ...
      error(thisClassTag,'???? Reference to field or method expected.')
    end % if
    
    
  case '{}'
    error(thisClassTag,'??? Cell contents reference from a non-cell array object.')
  ...
  case '.'
    ...
    switch index(1).subs
      ...
      otherwise
        error(thisClassTag,'???? Reference to non-existent field or method %s',index(1).subs)
    end % switch
end % switch
